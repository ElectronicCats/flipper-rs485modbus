...
#define FixedPaket                              \
  ((!app->modbus->slave && FUNCTION <= 0x06) || \
   (app->modbus->slave && FUNCTION >= 0x0F))
#define SLAVE        buf[0]
...
bool OpenLogFile(App* app) {
  ...
  if (storage_file_open(app->LOGfile, furi_string_get_cstr(selected_filepath),
...
#define WORKER_ALL_EVENTS \
  (WorkerEvtStop | WorkerEvtRxDone | WorkerEvtTxStart | WorkerEvtCfgChange)
...
static const char* functionNames[] = {"Read Coils(01)",
...
LL_USART_InitTypeDef buildUartSettings(Config* cfg) {
  ...
  USART_InitStruct.DataWidth = (cfg->dataWidth == 0   ? LL_USART_DATAWIDTH_7B
                                : cfg->dataWidth == 2 ? LL_USART_DATAWIDTH_9B
                                                      : LL_USART_DATAWIDTH_8B);

  USART_InitStruct.StopBits = (cfg->stopBits == 0   ? LL_USART_STOPBITS_0_5
                               : cfg->stopBits == 2 ? LL_USART_STOPBITS_1_5
                               : cfg->stopBits == 3 ? LL_USART_STOPBITS_2
                                                    : LL_USART_STOPBITS_1);

  USART_InitStruct.Parity = (cfg->parity == 1   ? LL_USART_PARITY_EVEN
...
static void Serial_Begin(FuriHalSerialHandle* handle,
                         LL_USART_InitTypeDef USART_InitStruct) {
...
void writeRingBuffer(RingBuffer* rb, uint8_t* buf, size_t len) {
  for (size_t i = 0; i < len; i++) {
    ...
    if (i == len - 1)
      rb->delimiters[rb->delimiterIdx] = rb->writeIdx;
    if (++rb->writeIdx > 255) {
      rb->delimiterIdx = 0;
      rb->writeIdx = 0;
    }
...
void discreteValuesParser(void* context,
                          uint8_t* buff,
                          size_t len,
                          FuriString* data) {
  App* app = context;
  ...
  while (len) {
    ...
    if (!app->uart->cfg->hexOutput) {
      ...
      for (int i = 0; i < 8; i++)
        furi_string_cat_printf(data, "%s%s", value >> i & 0x01 ? "ON" : "OFF",
                               i == 3   ? "\n->"
                               : i == 7 ? ""
                                        : ",");
    } else
...
void analogValuesParser(void* context,
                        uint8_t* buff,
                        size_t len,
                        FuriString* data) {
  App* app = context;
...
void pduParser(void* context,
  ...
  furi_string_cat_printf(
      ...
      SLAVE);
  memcpy(slave && FUNCTION <= 4 ? &bCount : &address, buf + offset,
         slave && FUNCTION <= 4 ? 1 : 2);

  offset += slave && FUNCTION <= 4 ? 1 : 2;
  address = address >> 8 | address << 8;
  if (app->uart->cfg->hexOutput)
    furi_string_cat_printf(
        data, slave && FUNCTION <= 4 ? "\nbCount: 0x%02X" : "\nAddress: 0x%04X",
...
void handle_rx_data_cb(uint8_t* buf, size_t len, void* context) {
  ...
  ///*
  furi_string_cat_printf(data, "\n-----%s----",
                         app->modbus->slave ? "PERIPHERAL-" : "---HUB----");
  ...
  if (app->LOGfileReady)
    storage_file_write(app->LOGfile, furi_string_get_cstr(data),
...
static void on_rx_cb(FuriHalSerialHandle* handle,
  ...
  if (ev & (FuriHalSerialRxEventData | FuriHalSerialRxEventIdle)) {
    ...
    while (size) {
      size_t ret =
          furi_hal_serial_dma_rx(handle, data,
                                 (size > FURI_HAL_SERIAL_DMA_BUFFER_SIZE)
...
static int32_t uart_worker(void* context) {
  App* app = context;
  while (1) {
    uint32_t events = furi_thread_flags_wait(WORKER_ALL_EVENTS, FuriFlagWaitAny,
                                             FuriWaitForever);
    ...
    if (events & WorkerEvtRxDone) {
      size_t len = furi_stream_buffer_receive(
          app->uart->rxStream, app->uart->rxBuff, RX_BUF_SIZE, 0);
      if (len > 0) {
...
void mainOptionsCB(void* context, uint32_t index) {
  ...
  switch (index) {
    case Settings_Option:
      scene_manager_set_scene_state(app->sceneManager, Main_Scene,
                                    Settings_Option);
    ...
    case Sniffer_Option:
      scene_manager_set_scene_state(app->sceneManager, Main_Scene,
                                    Sniffer_Option);
      scene_manager_set_scene_state(app->sceneManager, ConsoleOutput_Scene,
                                    Sniffer_Option);
  ...
  switch (selectedIndex) {
    ...
    case TimeOut_Option:
      ...
      variable_item_set_current_value_text(
          item, furi_string_get_cstr(app->modbus->timeout));
      break;
...
void CFG_Scene_OnEnter(void* context) {
  App* app = context;
  VariableItem* item;
  item = variable_item_list_add(app->varList, "Buadrate", BR_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->baudrate);
  variable_item_set_current_value_text(
      item, baudrateValues[app->uart->cfg->baudrate]);
  item = variable_item_list_add(app->varList, "Data size", DATAWIDTH_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->dataWidth);
  variable_item_set_current_value_text(
      item, dataWidthValues[app->uart->cfg->dataWidth]);
  item = variable_item_list_add(app->varList, "Stop bits", STOPBITS_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->stopBits);
  variable_item_set_current_value_text(
      item, stopBitsValues[app->uart->cfg->stopBits]);
  item = variable_item_list_add(app->varList, "Parity", PARITY_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->parity);
  variable_item_set_current_value_text(item,
                                       parityValues[app->uart->cfg->parity]);
  item = variable_item_list_add(app->varList, "TimeOut(ms)", TIMEOUT_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->timeout);
  furi_string_printf(app->modbus->timeout, "%d",
                     app->uart->cfg->timeout * TIMEOUT_SCALER);
  variable_item_set_current_value_text(
      item, furi_string_get_cstr(app->modbus->timeout));
  item = variable_item_list_add(app->varList, "Output Format",
                                DIGITALFORMAT_VALUES, itemChangedCB, app);
  variable_item_set_current_value_index(item,
                                        app->uart->cfg->hexOutput ? 1 : 0);
  variable_item_set_current_value_text(
      item, outputFormatValues[app->uart->cfg->hexOutput ? 1 : 0]);
  item = variable_item_list_add(app->varList, "Save LOG?", SAVE_LOG_VALUES,
                                itemChangedCB, app);
  variable_item_set_current_value_index(item, app->uart->cfg->saveLOG ? 1 : 0);
  variable_item_set_current_value_text(
      item, saveLOGValues[app->uart->cfg->saveLOG ? 1 : 0]);
...
void CFG_Scene_OnExit(void* context) {
  ...
  if (app->uart->cfg->saveLOG) {
    strcpy(app->logFilePath,
           sequential_file_resolve_path(app->storage, PATHLOGS, "Log", "log"));
    if (app->logFilePath != NULL) {
      if (storage_file_open(app->LOGfile, app->logFilePath, FSAM_WRITE,
                            FSOM_CREATE_ALWAYS)) {
        furi_string_reset(app->text);
...
void Sniffer_Scene_OnEnter(void* context) {
  App* app = context;
  serial_init(app->uart, UART_CH);
  if (scene_manager_get_scene_state(app->sceneManager, ConsoleOutput_Scene) ==
          Sniffer_Option ||
      scene_manager_get_scene_state(app->sceneManager, ConsoleOutput_Scene) ==
          Sender_Option) {
    text_box_set_font(app->textBox, TextBoxFontText);
    text_box_set_focus(app->textBox, TextBoxFocusEnd);
    furi_string_cat_printf(app->text, "Baudrate: %s",
                           baudrateValues[app->uart->cfg->baudrate]);
    furi_string_cat_printf(app->text, "\nData Width: %s",
                           dataWidthValues[app->uart->cfg->dataWidth]);
    furi_string_cat_printf(app->text, "\nStop bits: %s",
                           stopBitsValues[app->uart->cfg->stopBits]);
    furi_string_cat_printf(app->text, "\nParity: %s",
                           parityValues[app->uart->cfg->parity]);
    furi_string_cat_printf(app->text, "\nResponse TimeOut: %dms",
                           app->uart->cfg->timeout * TIMEOUT_SCALER);
  } else if (scene_manager_get_scene_state(
                 app->sceneManager, ConsoleOutput_Scene) == About_Option) {
    text_box_set_font(app->textBox, TextBoxFontText);
...
void SenderOptionsCB(void* context, uint32_t index) {
  ...
  if (index == Manual_Sender_Option) {
    scene_manager_set_scene_state(app->sceneManager, Sender_Scene,
                                  Manual_Sender_Option);
...
void Sender_Scene_OnEnter(void* context) {
  ...
  submenu_add_item(app->subMenu, "Manual Sender", Manual_Sender_Option,
                   SenderOptionsCB, app);
  submenu_add_item(app->subMenu, "Buffer Sender", Buffer_Sender_Option,
                   SenderOptionsCB, app);
  submenu_set_selected_item(app->subMenu, scene_manager_get_scene_state(
                                              app->sceneManager, Main_Scene));
  view_dispatcher_switch_to_view(app->viewDispatcher, Submenu_View);
...
void ByteInput_Scene_OnEnter(void* context) {
  ...
  uint8_t offset =
      scene_manager_get_scene_state(app->sceneManager, ByteInput_Scene);
...
//////////////////////////   Manual Sender Scene  //////////////////////////
const char* fns[] = {"0x01", "0x02", "0x03", "0x04",
                     "0x05", "0x06", "0x0F", "0x10"};
...
void itemEnterCB(void* context, uint32_t index) {
  ...
  uint8_t* buf = app->msgBuf;
  uint8_t SendButton =
      FUNCTION >= 0x0F ? (FUNCTION == 0x0F ? BYTECOUNT : QUANTITY) + 5 : 4;
...
void BuildValues(App* app, uint16_t byteCount, uint8_t* buf, bool one) {
  ...
  for (uint16_t i = 0; i < byteCount; i += one ? 1 : 2) {
    snprintf(lbl, sizeof(lbl), one ? "Byte %d" : "Register %d",
             one ? i + 1 : i / 2 + 1);
    snprintf(val, sizeof(val), one ? "0x%02X" : "0x%04X",
             one ? buf[i] : buf[i] << 8 | buf[i + 1]);
    item = variable_item_list_add(app->varList, strdup(lbl), 255, itemChangeCB,
                                  app);
...
void BuildSender(App* app, uint8_t* buf) {
  ...
  item = variable_item_list_add(app->varList, "Peripheral ID", 32, itemChangeCB,
                                app);
  variable_item_set_current_value_text(item, strdup(val));
  variable_item_set_current_value_index(item, SLAVE - 1);
  item = variable_item_list_add(app->varList, "Function", 8, itemChangeCB, app);
  variable_item_set_current_value_text(
      item, fns[FUNCTION <= 6 ? FUNCTION - 1 : FUNCTION - 9]);
  variable_item_set_current_value_index(
      item, FUNCTION <= 6 ? FUNCTION - 1 : FUNCTION - 9);
  Value = STARTADDRESS;
  snprintf(val, sizeof(val), "%d", Value);
  item = variable_item_list_add(app->varList, "Start Address", 255,
                                itemChangeCB, app);
  variable_item_set_current_value_text(item, strdup(val));
  ...
  if (FUNCTION != 0x05 && FUNCTION != 0x06) {
    ...
    snprintf(val, sizeof(val), "%d", Value);
    item = variable_item_list_add(app->varList, "Quantity", max, itemChangeCB,
                                  app);
    variable_item_set_current_value_text(item, strdup(val));
...
void Manual_Sender_Scene_OnEnter(void* context) {
  ...
  variable_item_list_set_selected_item(
      app->varList,
      scene_manager_get_scene_state(app->sceneManager, Manual_Sender_Scene));
  view_dispatcher_switch_to_view(app->viewDispatcher, VarList_View);
}
...
void OnItemEnterCB(void* context, uint32_t index) {
  ...
  for (uint8_t i = start; i <= app->ringBuffer->delimiters[index]; i++) {
    ...
    if (i == app->ringBuffer->delimiters[index])
      app->msgLen = i - start + 1;
  }
  scene_manager_next_scene(app->sceneManager, Manual_Sender_Scene);
}
void BuildCMDList(App* app) {
  ...
  do {
    len = 0;
    do {
      snprintf(lbl, sizeof(lbl), " %d | %d | %d |", SLAVE, FUNCTION,
               STARTADDRESS);
...
void (*const OnEnterHandlers[])(void*) = {
    Main_Scene_OnEnter,      CFG_Scene_OnEnter,
    Sniffer_Scene_OnEnter,   Sender_Scene_OnEnter,
    ByteInput_Scene_OnEnter, Manual_Sender_Scene_OnEnter,
    MSGsBuffer_Scene_OnEnter};
bool (*const OnEventHandlers[])(void*, SceneManagerEvent) = {
    Main_Scene_OnEvent,      CFG_Scene_OnEvent,
    Sniffer_Scene_OnEvent,   Sender_Scene_OnEvent,
    ByteInput_Scene_OnEvent, Manual_Sender_Scene_OnEvent,
    MSGsBuffer_Scene_OnEvent};
void (*const OnExitHandlers[])(void*) = {
    Main_Scene_OnExit,      CFG_Scene_OnExit,       Sniffer_Scene_OnExit,
    Sender_Scene_OnExit,    ByteInput_Scene_OnExit, Manual_Sender_Scene_OnExit,
...
static App* modbus_app_alloc() {
  ...
  view_dispatcher_set_navigation_event_callback(app->viewDispatcher,
                                                BackEventCB);
  view_dispatcher_set_tick_event_callback(app->viewDispatcher, ThickEventCB,
                                          100);
  app->subMenu = submenu_alloc();
  view_dispatcher_add_view(app->viewDispatcher, Submenu_View,
                           submenu_get_view(app->subMenu));
  app->varList = variable_item_list_alloc();
  view_dispatcher_add_view(app->viewDispatcher, VarList_View,
                           variable_item_list_get_view(app->varList));
  app->textBox = text_box_alloc();
  view_dispatcher_add_view(app->viewDispatcher, TextBox_View,
                           text_box_get_view(app->textBox));
  app->byteInput = byte_input_alloc();
  view_dispatcher_add_view(app->viewDispatcher, ByteInput_View,
...
int32_t Modbus_app(void* p) {
  ...
  view_dispatcher_attach_to_gui(app->viewDispatcher, gui,
